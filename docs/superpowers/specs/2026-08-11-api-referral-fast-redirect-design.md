...
同时，API 域入口不能等待 Vue UI 加载，不能公开或反代 Traffic 的 `8300` 端口，也不能先加载 `aiwelink.cc` 主页再进入注册页。
...
5. Traffic 识别 `entry=api` 后直接跳转 `https://api.aiwelink.cc/register`，不加载品牌主页。
6. 普通 `https://aiwelink.cc/r/{code}` 保持现有主页跳转行为。
7. 不接受任意 `next`、`redirect`、origin 或 URL 参数，避免开放重定向。
8. Sub2API 与 Traffic 分别使用独立功能分支和 PR；Sub2 PR 只投 `aiwelink-dev`，不修改或合并 Sub2 `main`。
...
浏览器
  ...
  <- 302 Location: https://api.aiwelink.cc/register
     Set-Cookie: awl_growth_sid=...; Domain=.aiwelink.cc; ...
  -> GET https://api.aiwelink.cc/register（自动携带父域 Cookie）
...
- 非 GET 方法不匹配该路由；
- 格式非法或功能未启用时，安全地 `302` 到本域相对路径 `/register`，不访问 Traffic。

相对 fallback 保证每个区域 API 域仍留在自己的注册页，不需要新增第二个公开 URL 配置。
...
Settings 增加固定 API 注册目标：

```dotenv
API_REGISTRATION_URL=https://api.aiwelink.cc/register
```
...
- `entry=api`：使用固定 `API_REGISTRATION_URL`；
...
当 `entry=api` 时，上述结果都直接到 API 注册页。只有成功的计数访问会写来源并设置/刷新 `awl_growth_sid`；无效码、排除请求或数据库错误不伪造 Cookie，继续沿用现有归因规则。
...
| Sub2 收到非法 code | `302` 到本域 `/register`，不访问 Traffic |
| Sub2 推广功能关闭 | `302` 到本域 `/register` |
| Traffic 收到合法 code | 记录来源、设置 Cookie、`302` 到 API 注册页 |
| Traffic 收到无效/停用/过期 code | 不设置新 Cookie，`302` 到 API 注册页 |
| Traffic 判定 bot/排除 | 保留现有审计规则，`302` 到 API 注册页 |
| Traffic 数据库故障 | 不伪造来源，`302` 到 API 注册页并记录稳定错误结果 |
...
1. API 域第一跳不访问前端资源；
2. Traffic 第二跳设置 Cookie 且 Location 为 API 注册页；
3. 浏览器最终 `/register` 请求携带 `awl_growth_sid`；
4. 新邮箱注册产生 outbox 并在 Traffic 锁定对应推广来源；
5. 整条链路不请求 `aiwelink.cc/` 主页文档。
