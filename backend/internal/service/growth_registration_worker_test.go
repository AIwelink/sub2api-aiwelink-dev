...
func TestGrowthRegistrationHTTPClientProxyRedirectAndBoundsPolicy(t *testing.T) {
	...
	require.NoError(t, err)
	privateTransport := privateClient.Transport.(*http.Transport)
	httpsTransport := httpsClient.Transport.(*http.Transport)
	...
	cloneTransport := clone.Transport.(*http.Transport)
